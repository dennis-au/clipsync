...
It starts, stops, and restarts the local `clipboard` and Cloudflare tunnel services. The menu also reports when the local clipboard is healthy but the tunnel is unavailable, and offers focused start or restart actions for `cloudflared`. Full-stack start uses `docker compose --profile tunnel up -d`; focused tunnel actions target only the `cloudflared` Compose service. It can copy the current shared password or generate a new cryptographically secure password and recreate the stack to apply it. Password rotation changes only `CLIPSYNC_PASSWORD` in `.env`; it never changes `compose.yaml` or removes the `clipboard-data` volume. Stop uses `docker compose stop`; it never removes stored ClipSync data.
